...
const splinterlandsUtils = (function () {
  ...
  function switch_rpc() {
    ...
    hive.api.setOptions({ transport: 'http', uri: rpc_node, url: rpc_node });
  ...
  function get_cur_block_num() {
    return Math.max(Math.floor((Date.now() - get_settings().timestamp) / 3000), 0) + get_settings().last_block;
  }

  function get_level(card) {
    if (card.edition >= 4 || card.details.tier >= 4) {
      const rates = get_settings()[card.gold ? 'combine_rates_gold' : 'combine_rates'][card.details.rarity - 1];
    ...
    for (let i = 0; i < get_settings().xp_levels[card.details.rarity - 1].length; i++) {
      if (card.xp < get_settings().xp_levels[card.details.rarity - 1][i]) {
        return i + 1;
      }
    }

    return get_settings().xp_levels[card.details.rarity - 1].length + 1;
  }

  function get_ecr(capture_rate, last_reward_block) {
    return Math.min((Number.isNaN(parseInt(capture_rate)) ? 10000 : capture_rate) + (get_settings().last_block - last_reward_block) * get_settings().dec.ecr_regen_rate, 10000);
  }

  function get_token(symbol) {
    return get_settings().supported_currencies.find((c) => c.currency == symbol);
  }

  function format_tx_id(id) {
    const prefix = get_settings().test_mode ? `${get_settings().prefix}sm_` : 'sm_';
    ...
    if (window.steem_keychain) {
      const result = await new Promise((resolve) =>
        steem_keychain.requestCustomJson(
          playerModule.get_player().name,
          get_settings().ssc.hive_chain_id,
          'Active',
      ...
      url += `&required_auths=${encodeURI(`["${playerModule.get_player().name}"]`)}`;
      url += `&id=${get_settings().ssc.hive_chain_id}`;
  ...
  function sc_custom_json(id, title, data, use_active) {
    let url = 'https://hivesigner.com/sign/custom-json?authority=active';
    url += `&required_posting_auths=${encodeURI(`[${use_active ? '' : `"${playerModule.get_player().name}"`}]`)}`;
    url += `&required_auths=${encodeURI(`[${use_active ? `"${playerModule.get_player().name}"` : ''}]`)}`;
    url += `&id=${id}`;
  ...
  async function browser_payment_available(token) {
    ...
    return new Promise(async (resolve) => {
      switch (token) {
        ...
        case 'EOS':
          return resolve(eos.hasIdentity());
        case 'ETH':
        case 'GAME':
        case 'BAT':
        case 'UNI':
  ...
  function asset_url(path) {
    return get_settings().asset_url + path;
  }

  function set_server_time_offset(serverTimeOffset) {
    _server_time_offset = serverTimeOffset;
  }

  function get_server_time_offset() {
    return _server_time_offset;
  }

  function server_date(date_str, subtract_seconds) {
    let date = new Date(new Date(date_str).getTime() + get_server_time_offset());
...
export default splinterlandsUtils;
