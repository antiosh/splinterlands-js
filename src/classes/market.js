...
splinterlands.Market = class {
  static async for_sale_by_card(card_detail_id, is_gold, edition) {
    const market_cards = await splinterlands.api('/market/for_sale_by_card', { card_detail_id, gold: is_gold, edition });
    return market_cards.map((c) => new splinterlands.MarketCard(c));
  }

  static async for_sale_grouped() {
    const market_cards_grouped = await splinterlands.api('/market/for_sale_grouped');
  ...
  static async purchase(market_ids, price, currency) {
    ...
    const tx = await splinterlands.ops.market_purchase(market_ids, price, currency);
    return tx;
  }
