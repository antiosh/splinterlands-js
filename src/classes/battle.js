...
splinterlands.Battle = class {
  constructor(data) {
    ...
    if (this.details.team1) {
      ...
      this.details.team1.monsters = this.details.team1.monsters ? this.details.team1.monsters.map((m) => new splinterlands.BattleCard(Object.assign(m, { team_num: 1 }))) : [];
    }
    ...
    if (this.details.team2) {
      ...
      this.details.team2.monsters = this.details.team2.monsters ? this.details.team2.monsters.map((m) => new splinterlands.BattleCard(Object.assign(m, { team_num: 1 }))) : [];
    }
  ...
  get ruleset_images() {
    return this.ruleset
      ? this.ruleset.map((r) => splinterlands.utils.asset_url(`website/icons/rulesets/new/img_combat-rule_${r.toLowerCase().replace(/[^a-zA-Z]+/g, '-')}_150.png`))
      : '';
  }

  static async load(id) {
    return new splinterlands.Battle(await splinterlands.api('/battle/result', { id }));
  }
