...
splinterlands.Player = class {
  constructor(data) {
    ...
    this.next_tier = !this.league.is_max_league ? new splinterlands.League(null, data.league + 1) : new splinterlands.League(null, data.league);
  ...
  async refresh() {
    const data = await splinterlands.api('/players/refresh');
    Object.keys(data).forEach((k) => (this[k] = data[k]));
  ...
  get ecr() {
    return Math.min(
      (Number.isNaN(parseInt(this.capture_rate)) ? 10000 : this.capture_rate) +
        (splinterlands.get_settings().last_block - this.last_reward_block) * splinterlands.get_settings().dec.ecr_regen_rate,
      10000,
    );
  }
  ...
  async update_avatar(avatar_id) {
    const response = await splinterlands.api('/players/set_avatar', { avatar_id });
  ...
  static async load(name) {
    ...
    return await new Promise(async (resolve, reject) => {
      const response = await splinterlands.api('/players/details', { name });
  ...
  async check_messages(type) {
    return await splinterlands.api('/players/messages', { type });
  }

  get max_cp_league() {
    let num = 0;

    for (let index = 0; index < splinterlands.get_settings().leagues.length; index++) {
  ...
  async external_cards(blockchain) {
    const res = await splinterlands.ec_api('/players/external_cards', { player: this.name, blockchain });
